...
namespace Solana.Unity.SDK.Example
{
    public class WalletScreen : SimpleScreen
    {
        ...
        public void Start()
        {
            ...
            {
                manager.ShowScreen(this, "other_screen");
            });
            ...
            {
                Debug.Log(StateManager.Instance.State?.CurrentUser?.ToString());
                Debug.Log(StateManager.Instance.State?.CurrentProfile?.ToString());
                if (StateManager.Instance.State.CurrentUser != null && StateManager.Instance.State.CurrentProfile != null)
                {
                    manager.ShowScreen(this, "profile_screen");
                }
                else
                {
        ...
        private async void OnEnable()
        {
            ...
            try
            {
                ...
                var UsersReturn = await StateManager.Instance.Client.FindUsers(new FindUsersParams
                {
                    Wallets = new List<string> { Web3.Account.PublicKey.ToString() },
                    IncludeProof = false

                });

                if (UsersReturn.User != null && UsersReturn.User.Count > 0)
                {
                    StateManager.Instance.SaveUser(UsersReturn.User[0]);

                    var ProfilesReturn = await StateManager.Instance.Client.FindProfiles(new FindProfilesParams
                    {
                        UserIds = UsersReturn.User.Select(u => u.Id).ToList(),
                        Projects = new List<string> { StateManager.Instance.State.Project }
                    });
                    if (ProfilesReturn.Profile != null && ProfilesReturn.Profile.Count > 0)
                    {
                        Debug.Log(ProfilesReturn.Profile[0].Info.Name);
                        StateManager.Instance.SaveProfile(ProfilesReturn.Profile[0]);
                    }
                }
            }
            catch (Exception ex)
