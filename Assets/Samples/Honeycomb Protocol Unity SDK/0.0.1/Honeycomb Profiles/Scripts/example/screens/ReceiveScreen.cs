...
public class ReceiveScreen : SimpleScreen
{
    ...
    {
        var isHoneynet = IsHoneynet();
        airdrop_btn.enabled = isHoneynet;
        airdrop_btn.interactable = isHoneynet;
    }
    ...
    private async void RequestAirdrop()
    {
        ...
        else
        {
            ...
            Debug.Log("Airdrop success, see transaction at https://explorer.solana.com/tx/" + result.Result + "?cluster=custom&customUrl=https://rpc.test.honeycombprotocol.com");
            manager.ShowScreen(this, "wallet_screen");
        }
        Loading.StopLoading();
    }

    private static bool IsHoneynet()
    {
        return Web3.Rpc.NodeAddress.AbsoluteUri.Contains("test");
    }
